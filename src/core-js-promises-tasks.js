  ...
  return new Promise((resolve, reject) => {
    ...
    promises.forEach((p) => {
      p.then(resolve).catch(() => {
        rejectedCount = +1;
        if (rejectedCount === total) reject(new Error('All promises rejected'));
      });
    });
  });
}
...
function getAllResult(promises) {
  return Promise.all(
    promises.map((p) => p.then((val) => val).catch(() => null))
  );
}
...
function queuePromises(promises) {
  return promises.reduce((acc, curr) => {
    return acc.then((resultStr) => {
      return curr.then((val) => resultStr + val);
    });
  }, Promise.resolve(''));
}
